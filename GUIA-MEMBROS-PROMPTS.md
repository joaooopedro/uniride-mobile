...
2. **Clonar o projeto**:
   ...
   npm install
   ```
3. **Criar a sua branch de trabalho, clicando na task e depois create a branch, dai é so clonar localmente com o git desktop ou terminal ou git cli.![[Pasted image 20260925112658.png]]**
   ```bash
...
6. **Testar no Expo Go**:
   ...
   npx expo start
   ```
   Baixa o App e abra o app Expo Go no celular (Android ou iOS) ou emulador e confira a tela e navegação. DEPOIS DE DAR O EXPO START É SO LER O QRCODE.
7. **Commitar e Enviar**:
Commite de acordo, gere o pull request e mergeie a branch quando estiver tudo completo.
...
=== DIRETRIZES ANTI-SLOP (PROIBIÇÕES RÍGIDAS DE CÓDIGO, UI E TEXTO) ===
1. Código Limpo (Sem Slop de IA):
   - NUNCA crie comentários óbvios narrando a linha de código abaixo.
   - NUNCA use nomes genéricos de variáveis ou estados (ex: data, result, temp, processedData, item, res). Use sempre nomes contextuais e semânticos (ex: caronasDisponiveis, filtroCampusSelecionado, passageiroAtual).
   - NUNCA utilize blocos try/catch em cascata desnecessários, checagens defensivas sem caso real ou abstrações criadas para um único uso.
   - NUNCA duplique componentes existentes.
2. Interface e Estilização Sóbria (Sem Slop Visual):
   - NUNCA adicione barras ou linhas divisórias horizontais decorativas entre cada bloco ou tópico.
   - NUNCA use bordas ou cards com cores exageradas, gradientes de fundo artificiais ou efeitos desnecessários. O design deve ser sóbrio, limpo, moderno e funcional.
   - NUNCA use cores decorativas sem função semântica nos textos e títulos; use estritamente as constantes do tema (`theme.colors`).
   - NUNCA insira ícones decorativos sorteados sem utilidade funcional real (como estrelas, foguetes ou brilhos aleatórios).
   - Empty states devem ser objetivos, informativos e funcionais, orientando a ação correta.
3. Redação e Copy Direto (Sem Clichês de IA):
   - Textos e mensagens devem ser sempre diretos, técnicos e concisos.
   - NUNCA utilize clichês de IA e copy de enfeite (ex: "solução completa", "de forma simples e eficiente", "mergulhar fundo", "no cenário atual", "alavancar", "experiência única", "de ponta a ponta", "robusto", "game changer", "sem esforço").
   - NUNCA utilize travessões desnecessários ou emojis aleatórios.
```
